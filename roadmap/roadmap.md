...
- False-positive delay warnings erode trust faster than no warning at all — this is the reason the guardrail metric exists, not an afterthought. (Correction from the critique pass in `analysis/limitations-and-alternative-views.md`: this risk isn't symmetric with a false negative. Today's baseline is a guaranteed false negative on every late order, the worst cell in the data — so "erode trust" cuts both ways and the guardrail should say which direction to err, not just "stay under a threshold.")

## Parallel workstream worth naming, not currently in v1-v3

`analysis/limitations-and-alternative-views.md`'s business-strategy lens found something the state-level lateness table doesn't make obvious by itself: raw percentage-late by state is dominated by low-volume states (Alagoas' 21.4% is only ~85 late orders), while **São Paulo and Rio de Janeiro together account for roughly half of all late orders nationally by sheer volume**, not rate. SP delivers 40,494 orders at 4.5% late (~1,822 late orders); RJ delivers 12,350 at 12.1% late (~1,494 late orders) — nearly as many late orders from a third of the volume. That reframes a diffuse "fix logistics everywhere" mandate into a scoped, answerable question: why does RJ underperform SP by nearly 3x on lateness rate at similar market maturity?

This is explicitly **not scoped into v1-v3 above** — those are all customer-communication features, and this is an ops/logistics diagnostic, a different kind of work with a different owner. It's named here because it has a plausible higher ceiling than the notification feature (fixing the actual delay beats communicating about it) and costs very little to scope: a short investigation into whether Olist or its sellers control carrier selection in RJ, and whether an alternate carrier or route exists, could tell the team within a sprint whether a higher-leverage fix is available. Recommended sequencing: run this diagnostic in parallel with v1, not before it — v1 is cheap and helps regardless of what the diagnostic finds, so there's no reason to block it on an ops investigation with an unknown timeline.
